...
def parse_console_url(console_url: str) -> Tuple[str, str, int]:
    ...
    # Prefer parsed.hostname to avoid including a :port in the worker key.
    # (Some deployments use wss://workerXX:443/..., while services typically report host without a port.)
    worker_hostname = parsed.hostname or parsed.netloc
...
def iter_ssh_config_files() -> List[Path]:
    """
    Return all candidate SSH config files under the repo's .ssh directory.

    This repo often stores configs under .ssh/<group>/<sim-name>, so we search
    recursively rather than only looking at direct children.
    """
...
def find_ssh_config_files_for_sim_id(sim_id: str) -> List[Path]:
    """Find all SSH config files that declare the given Simulation ID comment."""
    matches: List[Path] = []
    for p in iter_ssh_config_files():
        try:
            _, file_sim_id, _ = parse_ssh_config(p)
        except Exception:
            continue
        if file_sim_id and file_sim_id.strip() == sim_id.strip():
            matches.append(p)
    return matches


AIR_WORKER_BLOCK_BEGIN = "# BEGIN AIR WORKER HOSTS (auto-generated by scripts/vnc-console-connect.py)"
AIR_WORKER_BLOCK_END = "# END AIR WORKER HOSTS"


def strip_air_worker_block(content: str) -> str:
    """Remove the auto-generated Air worker host block if present."""
    if AIR_WORKER_BLOCK_BEGIN not in content:
        return content
    start = content.find(AIR_WORKER_BLOCK_BEGIN)
    if start < 0:
        return content
    end = content.find(AIR_WORKER_BLOCK_END, start)
    if end < 0:
        # If the end marker is missing, conservatively keep content unchanged.
        return content
    end = end + len(AIR_WORKER_BLOCK_END)
...
def generate_air_worker_block(
    workers: Dict[str, Optional[str]],
    guessed_workers: Set[str],
    ssh_user: str,
    identity_file: Optional[str],
) -> str:
    """
    Generate an SSH config block mapping Air worker hostname -> SSH port.

    workers: worker hostname -> port string (or None if unknown)
    guessed_workers: hosts whose port was guessed (not reported by the services API)
    """
    lines: List[str] = []
    lines.append(AIR_WORKER_BLOCK_BEGIN)
    lines.append(f"# Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("#")
    lines.append("# These hosts are used as SSH tunnel endpoints for VNC console forwarding.")
    lines.append("# Ports are sourced from the Air 'services' API when possible; otherwise they may be guessed.")
    lines.append("")

    for host in sorted(workers.keys()):
        port = workers.get(host)
        ...
        if port:
            ...
            lines.append("  # Port: MISSING (no SSH service found for this worker)")
        if identity_file:
        ...
        if host in guessed_workers:
            lines.append("  # NOTE: Port was guessed; publish an SSH service for this worker for a reliable port.")
        lines.append("")

    # Remove trailing blank line for a cleaner block.
    while lines and lines[-1] == "":
        lines.pop()
    lines.append(AIR_WORKER_BLOCK_END)
    return "\n".join(lines) + "\n"


def update_ssh_config_with_air_workers(
    config_path: Path,
    worker_ports: Dict[str, Optional[str]],
    guessed_workers: Set[str],
) -> bool:
    """
    Update an SSH config file by writing the auto-managed Air worker host block.

    Returns True if the file contents changed.
    """
    content = config_path.read_text(encoding="utf-8")
    bcm_cfg = find_bcm_host_config(content) or {}
    ssh_user = str(bcm_cfg.get("user") or "ubuntu").strip() or "ubuntu"
    identity_file = bcm_cfg.get("identity_file")

    cleaned = strip_air_worker_block(content)
    block = generate_air_worker_block(worker_ports, guessed_workers, ssh_user, identity_file)
...
def match_simulation_to_ssh_config(simulations: List[dict]) -> Tuple[Optional[dict], Optional[Path], Optional[dict]]:
    ...
    for config_file in iter_ssh_config_files():
...
def print_vnc_info(
    ...
    for node in nodes:
        ...
        if not display_hostname or not ssh_port:
            ...
            if node_worker:
                print(f"    Console worker: {node_worker}")
            if ssh_ports_by_worker:
                known = ", ".join(
                    f"{host}:{port}" for host, port in sorted(ssh_ports_by_worker.items(), key=lambda kv: kv[0])
                )
                print(f"    SSH service workers: {known}")
            else:
                print("    SSH service workers: (none found via API)")
            print("    Fix: publish an SSH service for the console worker in Air (one per worker for multi-worker sims).")
            continue
        if node_worker and node_worker in guessed_workers:
            print(f"  ⚠ No SSH service reported for {node_worker}; using a guessed SSH port ({ssh_port})")
        elif node_worker and node_worker not in ssh_ports_by_worker and default_ssh_port:
...
def main() -> int:
    ...
    try:
        ...
        # If we have a sim_id, try to locate any repo SSH config(s) for it.
        # This repo stores configs under .ssh/<group>/<sim-name>, so we search by Simulation ID.
        config_candidates = find_ssh_config_files_for_sim_id(sim_id)
        if config_candidates:
            # Prefer a candidate whose declared simulation name matches what Air reports, if known.
            # Otherwise choose the newest file (mtime) as the primary config.
            primary = max(config_candidates, key=lambda p: p.stat().st_mtime)
            config_path = primary
            try:
                _, _, cfg_content = parse_ssh_config(primary)
                bcm_config = find_bcm_host_config(cfg_content)
                if bcm_config:
                    ssh_service_port = bcm_config.get("port")
                    worker_hostname = bcm_config.get("hostname")
            except Exception:
        ...
        # As a last resort, pick any SSH service port we do have. This enables best-effort tunnels
        # on multi-worker simulations even when only one worker has an SSH service published.
        if not default_ssh_port and ssh_ports_by_worker:
            default_ssh_port = next(iter(sorted(ssh_ports_by_worker.items(), key=lambda kv: kv[0])))[1]

        # Compute console workers and fill missing worker ports (guessed when needed).
        console_workers: Set[str] = set()
        for node in nodes:
            console_url = (node or {}).get("console_url", "")
            console_port = (node or {}).get("console_port")
            if not console_url or not console_port:
                continue
            w, _, _ = parse_console_url(console_url)
            if w:
                console_workers.add(w)

        guessed_workers: Set[str] = set()
        effective_worker_ports: Dict[str, str] = dict(ssh_ports_by_worker)
        missing_workers = sorted(w for w in console_workers if w not in effective_worker_ports)
        if missing_workers and default_ssh_port:
            for w in missing_workers:
                effective_worker_ports[w] = default_ssh_port
                guessed_workers.add(w)
        ...
        # Default mode: if we have missing workers and can find SSH config(s), update them.
        if missing_workers and config_candidates:
            # Write a block containing all console workers, with guessed ports where needed.
            worker_ports_for_config: Dict[str, Optional[str]] = {w: effective_worker_ports.get(w) for w in sorted(console_workers)}
            for p in config_candidates:
                if update_ssh_config_with_air_workers(p, worker_ports_for_config, guessed_workers):
                    updated_configs.append(p)
        
        # Default mode: print VNC info
        print_vnc_info(nodes, effective_worker_ports, guessed_workers, default_worker, default_ssh_port)
        if updated_configs:
            print("SSH config updated with missing Air workers:")
            for p in updated_configs:
                print(f"  - {p}")
